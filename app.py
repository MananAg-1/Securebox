...
def list_files():
    ...
    # Fetch only files that belong to the logged-in user
    cursor.execute("""
        SELECT filename FROM file_descriptions WHERE user_id = ?
    """, (current_user.id,))
    user_files = [row['filename'] for row in cursor.fetchall()]
    conn.close()
    ...
    for filename in user_files:
        ...
        # Skip if the file does not exist (for consistency)
